...
interface Props {
  data: TechCategory[] | FlatTechItem[];
}

/**
 * Normaliza os dados técnicos: se vierem no formato flat { category, label, value }[],
 * agrupa-os por categoria no formato TechCategory[].
 */
function normalizeData(data: TechCategory[] | FlatTechItem[]): TechCategory[] {
  if (!data || data.length === 0) return [];

  // Detectar formato flat: se o primeiro item tem "label" e "value" como strings
  const first = data[0] as unknown as Record<string, unknown>;
  if (typeof first.label === "string" && typeof first.value === "string") {
    const grouped: Record<string, Record<string, string>> = {};
    for (const item of data as FlatTechItem[]) {
      const cat = item.category || "Geral";
      if (!grouped[cat]) grouped[cat] = {};
      grouped[cat][item.label] = item.value;
    }
    ...
    <div className="bg-white py-16 md:py-24">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="flex flex-wrap justify-center gap-6 lg:gap-8">
          {normalizedData.map((category, idx) => {
