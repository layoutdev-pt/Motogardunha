...
function TimelineStep({ item, idx, isLast }: { item: TimelineItem; idx: number; isLast: boolean }) {
    ...
    <section className="py-20 bg-background-alt">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="space-y-8">
          {items.map((item, idx) => (
            <TimelineStep key={item.step} item={item} idx={idx} isLast={idx === items.length - 1} />
          ))}
        </div>
      </div>
    </section>
